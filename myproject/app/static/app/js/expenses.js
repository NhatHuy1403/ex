...
((UIController, ExpenseController) => {
    ...
    init();
    const expenseDateInput = document.getElementById("input-expense-date");
const expenseMonthInput = document.getElementById("input-expense-month");
const expenseYearInput = document.getElementById("input-expense-year");

document.getElementById("btn-submit-expense").addEventListener("click", function () {
  const description = document.getElementById("input-expense-description").value;
  const value = parseFloat(document.getElementById("input-expense-value").value);
  const date = expenseDateInput.value;
  const month = expenseMonthInput.value;
  const year = parseInt(expenseYearInput.value);

  if (isNaN(value) || description.trim().length === 0) {
